...
export async function toggleLevelSystem(groupId, active) {
  try {
    const { data, error } = await supabase
      .from('group_settings')
      .upsert(
        { group_id: groupId, level_system_active: active },
        { onConflict: 'group_id' }
      )
      .select()
      .single();
  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        // Se o registro não existir, retorna false (sistema não ativo)
        return false;
      }
      throw error;
    }
    ...
    if (levelData) {
      ...
      const { data: updatedData, error: updateError } = await supabase
        ...
        .update({
          ...
          username: username // Atualiza o username caso tenha mudado
        })
        .eq('id', levelData.id)
        .select()
        .single();
      ...
      return 1; // Novo usuário, retorna o nível inicial
    }
  } catch (error) {
...
export async function validateAndCorrectUserLevels() {
  try {
    ...
    for (const user of allUsers) {
      const correctLevel = Math.max(1, calculateLevelFromXP(user.xp));
      if (user.level !== correctLevel) {
        ...
        if (updateError) {
          console.error(`Erro ao corrigir nível do usuário ${user.username}:`, updateError);
        } else {
          console.log(`Nível corrigido para o usuário ${user.username}: de ${user.level} para ${correctLevel}`);
        }
      }
    }

    console.log('Validação e correção de níveis concluída');
  } catch (error) {
    console.error('Erro durante a validação e correção de níveis:', error);
  }
}